...
<? if ($dataProvider->query->count()) : ?>
    ...
    <p>Всего куплено товаров (без учета доставки):
        <b><?= \Yii::$app->money->convertAndFormat($moneyTotalNoDelivery); ?></b></p>
    <p>Всего куплено товаров (закупка): <b><?= \Yii::$app->money->convertAndFormat($moneyPurchasing); ?></b></p>
    <p>Закупка - Реальная цена продажи:
        <b><?= \Yii::$app->money->convertAndFormat($moneyTotalNoDelivery->subtract($moneyPurchasing)); ?></b></p>
    <p>Минус коммисия v3tosy.ru (50%): <b
                style="font-size: 20px;"><?= \Yii::$app->money->convertAndFormat($moneyTotalNoDelivery->subtract($moneyPurchasing)->multiply(0.5)); ?></b>
        <small>грязный доход аффилиата</small>
    </p>
    <hr/>
    <? if ($notFoundProducts) : ?>
        Не найдены некоторые товары: <?= $notFoundProducts; ?>
        <small>(Влияет на стоимость закупки)</small>
    <? endif; ?>
<? endif; ?>
